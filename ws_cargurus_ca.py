...
from utils.parsers.json import JsonFiles
from utils.connections.netops.proxies.managers.free import YieldFreeProxy
...
class CarGurusCa:
  ...
  def get_infos_mode_1(self, pg_number: int) -> Dict[str, Any]:
    ...
    if session is None:
      req_resp = request("GET", url, headers=headers, data=payload)
    else:
      req_resp = session.get(url, headers=headers, data=payload)
    req_resp.raise_for_status()
    return req_resp.json()

if __name__ == "__main__":
    cls_cargurus = CarGurusCa()
    for pg_number in range(1, 436):
